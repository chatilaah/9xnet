...
using static W9xNET.Interop.Win32.Shell32;
...
namespace W9xNET.RcReader
{
    ...
    internal static class RcHelper
    {
        ...
        /// <summary>
        /// Returns W9xNET.RcReader.IconInfo object that holds the information about the icon.
        /// </summary>
        /// <param name="icon">System.Drawing.Icon to get the information about.</param>
        /// <returns>W9xNET.RcReader.IconInfo object that holds the information about the icon.</returns>
        ...
        public static Icon ExtractIcon(string fileName, int iconIndex)
        {
            ...
            using (Extractor extractor = new Extractor(fileName))
            {
                return extractor.GetIconAt(iconIndex);
            }
        }

        public static int ExtractAllDialogs(string fileName)
        {
            //Icon icon = null;
            ...
            //Load the file as an executable module.
            using (var extractor = new Extractor(fileName))
            {
                for (int i = 0; i < extractor.DialogCount; i++)
                {
                    list.Add(extractor.GetDialogAt(i));
                }
            }
            //return list;
            return 1;
        ...
        public static Icon? GetAssociatedIcon(string fileName, IconFlags flags)
        {
            ...
            IntPtr result = Interop.Win32.Shell32.SHGetFileInfo(fileName, 0, ref fileInfo, (uint)Marshal.SizeOf(fileInfo), (SHGetFileInfoFlags)flags);
        ...
        public static Icon Merge(params Icon[] icons)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                for (int j = 0; j < iconInfo.Images.Count; j++)
                {
                    ...
                    outputStream.Write(iconInfo.RawData[j], 0, (int)entry.BytesInRes);

                    //Update the imageIndex and the imageOffset
                    imageIndex++;
                    imageOffset += (int)entry.BytesInRes;
                }
            }
